...
async def get_whatsapp_settings(mask: bool = False) -> Dict[str, Any]:
    ...
    settings = {**default_whatsapp_settings(), **(settings or {})}
    if mask:
...
async def forgot_password(payload: ForgotPasswordInput, background_tasks: BackgroundTasks):
    ...
    request_doc = {
        ...
        "status": "account_found" if user else "account_not_found",
    ...
    if user:
        ...
        request_doc.update({"status": "otp_created", "otp_id": reset_id, "expires_at": expires_at})
...
async def dashboard(_: Dict[str, Any] = Depends(require_admin)):
    ...
    courses = await db.courses.find({"status": "active"}, {"_id": 0}).to_list(500)
    ...
    missing = 0
    for assignment in published_assignments:
        class_doc = next((c for c in classes if c["id"] == assignment.get("class_id")), None)
        if class_doc:
            for student_id in class_doc.get("student_ids", []):
    ...
    risk_high = 0
    for student in students:
        progress = await calculate_student_progress(student["id"])
        if progress["risk_label"] == "Risiko Tinggi":
            risk_high += 1
    return {
        "summary": {
...
CLEAN_DATA_MODULES = {
    ...
    "notifications": {
        ...
        "description": "Menghapus reminder in-app, histori WhatsApp, OTP, dan request reset password.",
    },
    "all": {
...
async def class_students(class_id: str, _: Dict[str, Any] = Depends(require_admin)):
    ...
    for student in students:
        student["progress"] = await calculate_student_progress(student["id"])
...
async def list_materials(user: Dict[str, Any] = Depends(get_current_user)):
    ...
    for material in materials:
        ...
        enriched.append(await enrich_material_payload(material))
    return enriched
...
async def list_comments(material_id: str, user: Dict[str, Any] = Depends(get_current_user)):
    ...
    for comment in comments:
        ...
        if author_id not in displayed_admin_names:
            author = await db.users.find_one({"id": author_id}, {"_id": 0}) or {
                "id": author_id,
                "role": "admin",
    ...
    if user["role"] == "student":
        ...
        for assignment in assignments:
            submission = await db.submissions.find_one(
                {"assignment_id": assignment["id"], "student_id": user["id"]}, {"_id": 0}
            )
            if submission:
                enrich_submission_file_urls(submission)
