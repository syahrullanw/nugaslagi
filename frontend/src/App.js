...
import {
  AlertTriangle, ArrowLeft, BarChart3, Bell, BookOpen, CalendarDays, CheckCircle2, ClipboardList, Download,
  Eye, FileText, FileSpreadsheet, GraduationCap, LayoutDashboard, LogOut, Mail, MessageSquare, Pencil, Plus, Send,
...
const DeadlineCountdown = memo(function DeadlineCountdown({ deadline, testid, compact = false }) {
...
function LoginScreen({ onAuth, branding }) {
  ...
  const [forgot, setForgot] = useState({ identifier: resetQuery.identifier, otp: "", new_password: "", sent: resetQuery.active, message: resetQuery.active ? "Masukkan OTP dari WhatsApp atau email dan password baru." : "", delivery: null, emailDelivery: null });
  ...
  return <main className="min-h-screen bg-slate-50 lg:grid lg:grid-cols-[1.05fr_0.95fr]" data-testid="login-screen">
    <section className="relative hidden overflow-hidden bg-slate-950 lg:block" data-testid="login-visual-section"><img src={authBg} alt="E-learning akademik" className="h-full w-full object-cover opacity-75" loading="lazy" data-testid="login-background-image" /><div className="absolute inset-0 bg-gradient-to-t from-slate-950 via-slate-950/35 to-transparent" /><div className="absolute bottom-12 left-12 right-12 text-white"><img src={brandingLogo(branding)} alt={`Logo ${brandingName(branding)}`} className="mb-8 h-16 w-16" data-testid="login-logo-image" /><h1 className="font-display text-5xl font-bold leading-tight" data-testid="login-hero-title">Ruang kendali pembelajaran dosen.</h1><p className="mt-5 max-w-xl text-lg text-slate-200" data-testid="login-hero-subtitle">Kelola kelas, tugas, submission, rubrik, reminder, dan rekap nilai dari satu aplikasi PWA.</p></div></section>
    <section className="flex min-h-screen items-center px-5 py-10 md:px-12" data-testid="login-form-section"><div className="mx-auto w-full max-w-xl animate-rise"><div className="mb-6 flex items-center gap-3"><img src={brandingLogo(branding)} alt="Logo" className="h-12 w-12" data-testid="login-mobile-logo-image" /><div><p className="text-xs font-semibold uppercase tracking-[0.2em] text-slate-500" data-testid="login-overline">{brandingName(branding)}</p><h2 className="font-display text-3xl font-bold text-slate-950" data-testid="login-title">{mode === "login" ? "Satu pintu login" : mode === "register" ? "Daftar mahasiswa" : "Lupa password"}</h2></div></div>
      ...
      {mode === "forgot" && <form onSubmit={forgot.sent ? submitResetOtp : submitForgot} className="space-y-5 border border-slate-200 bg-white p-6" data-testid="forgot-password-form"><Field id="forgot-identifier" label="Username / NIM / Nomor HP / Email"><Input id="forgot-identifier" data-testid="forgot-password-identifier-input" value={forgot.identifier} onChange={(e) => setForgot({ ...forgot, identifier: e.target.value })} /></Field>{forgot.message && <p className={`rounded-md border p-3 text-sm ${forgot.sent ? "border-blue-200 bg-blue-50 text-blue-800" : "border-red-200 bg-red-50 text-red-800"}`} data-testid="forgot-password-message">{forgot.message}</p>}{forgot.delivery && <div className={`rounded-md border p-3 text-sm ${otpDeliveryClass(forgot.delivery.status)}`} data-testid="forgot-password-otp-queue-status"><p className="font-semibold">Antrian pesan WhatsApp</p><p className="mt-1">{otpDeliveryText(forgot.delivery)}</p></div>}{forgot.emailDelivery && <div className={`rounded-md border p-3 text-sm ${forgot.emailDelivery.status === "sent" ? "border-emerald-200 bg-emerald-50 text-emerald-800" : forgot.emailDelivery.status === "failed" ? "border-red-200 bg-red-50 text-red-800" : "border-slate-200 bg-slate-50 text-slate-600"}`} data-testid="forgot-password-email-status"><p className="font-semibold">Email OTP</p><p className="mt-1">{forgot.emailDelivery.status === "sent" ? "OTP sudah dikirim ke email Anda." : forgot.emailDelivery.status === "failed" ? `Gagal mengirim email: ${forgot.emailDelivery.error || "Error tidak diketahui"}` : forgot.emailDelivery.status === "no_email" ? "Email tidak terdaftar pada akun ini." : "Status email tidak diketahui."}</p></div>}{forgot.sent && <><Field id="forgot-otp" label="OTP"><Input id="forgot-otp" data-testid="forgot-password-otp-input" value={forgot.otp} onChange={(e) => setForgot({ ...forgot, otp: e.target.value })} /></Field><Field id="forgot-new-password" label="Password baru"><Input id="forgot-new-password" type="password" data-testid="forgot-password-new-input" value={forgot.new_password} onChange={(e) => setForgot({ ...forgot, new_password: e.target.value })} /></Field></>}<Button className="w-full" disabled={busy} data-testid="forgot-password-submit-button"><Send /> {forgot.sent ? "Reset password" : "Kirim OTP"}</Button><p className="text-sm text-slate-500" data-testid="forgot-password-help">OTP dikirim via WhatsApp dan/atau email tergantung konfigurasi gateway.</p></form>}
    </div></section>
  </main>;
}

const StatCard = memo(function StatCard({ icon: Icon, label, value, hint, testid }) {
...
function WhatsAppPage({ forms, setForms, saveWhatsApp, messages, retryMessage }) {
  ...
  return <div className="space-y-6" data-testid="whatsapp-page"><form onSubmit={saveWhatsApp} className="space-y-4 border bg-white p-5" data-testid="whatsapp-settings-form"><h2 className="font-display text-2xl font-semibold" data-testid="whatsapp-settings-title">Konfigurasi WhatsApp Gateway</h2><div className="grid gap-4 md:grid-cols-4"><Field id="whatsapp-provider" label="Provider"><select id="whatsapp-provider" className="form-select" data-testid="whatsapp-provider-select" value={w.provider || "disabled"} onChange={(e) => updateWhatsApp({ provider: e.target.value })}><option value="disabled">Nonaktif</option><option value="fonnte">Fonnte</option><option value="waha">WAHA</option></select></Field><Field id="whatsapp-app-url" label="Link reset/aplikasi"><Input id="whatsapp-app-url" data-testid="whatsapp-app-url-input" value={w.app_url || ""} onChange={(e) => updateWhatsApp({ app_url: e.target.value })} placeholder="https://domain-aplikasi" /></Field><Field id="whatsapp-send-delay" label="Delay kirim (detik)"><Input id="whatsapp-send-delay" type="number" min="0" max="300" data-testid="whatsapp-send-delay-input" value={w.send_delay_seconds ?? 3} onChange={(e) => updateWhatsApp({ send_delay_seconds: Number(e.target.value) })} /></Field><Field id="whatsapp-typing-sim" label="Simulasi mengetik (detik)"><Input id="whatsapp-typing-sim" type="number" min="0" max="120" data-testid="whatsapp-typing-sim-input" value={w.typing_simulation_seconds ?? 30} onChange={(e) => updateWhatsApp({ typing_simulation_seconds: Number(e.target.value) })} /></Field></div>{w.provider === "fonnte" && <div className="grid gap-4 md:grid-cols-2" data-testid="fonnte-config-fields"><Field id="fonnte-token" label="Token Fonnte"><Input id="fonnte-token" type="password" data-testid="fonnte-token-input" value={w.fonnte_token || ""} onChange={(e) => updateWhatsApp({ fonnte_token: e.target.value })} placeholder={w.fonnte_token_masked || "Token Fonnte"} /></Field><Field id="fonnte-url" label="URL API Fonnte"><Input id="fonnte-url" data-testid="fonnte-url-input" value={w.fonnte_url || "https://api.fonnte.com/send"} onChange={(e) => updateWhatsApp({ fonnte_url: e.target.value })} /></Field></div>}{w.provider === "waha" && <div className="grid gap-4 md:grid-cols-3" data-testid="waha-config-fields"><Field id="waha-base-url" label="WAHA Base URL"><Input id="waha-base-url" data-testid="waha-base-url-input" value={w.waha_base_url || ""} onChange={(e) => updateWhatsApp({ waha_base_url: e.target.value })} /></Field><Field id="waha-api-key" label="WAHA API Key"><Input id="waha-api-key" type="password" data-testid="waha-api-key-input" value={w.waha_api_key || ""} onChange={(e) => updateWhatsApp({ waha_api_key: e.target.value })} placeholder={w.waha_api_key_masked || "X-Api-Key"} /></Field><Field id="waha-session" label="Session"><Input id="waha-session" data-testid="waha-session-input" value={w.waha_session || "default"} onChange={(e) => updateWhatsApp({ waha_session: e.target.value })} /></Field></div>}<div className="grid gap-4 xl:grid-cols-2"><Field id="whatsapp-template" label="Template OTP"><Textarea id="whatsapp-template" data-testid="whatsapp-template-input" value={w.otp_template || ""} onChange={(e) => updateWhatsApp({ otp_template: e.target.value })} /></Field><Field id="whatsapp-assignment-template" label="Template tugas baru"><Textarea id="whatsapp-assignment-template" data-testid="whatsapp-assignment-template-input" value={w.assignment_template || ""} onChange={(e) => updateWhatsApp({ assignment_template: e.target.value })} /></Field><Field id="whatsapp-grade-template" label="Template nilai"><Textarea id="whatsapp-grade-template" data-testid="whatsapp-grade-template-input" value={w.grade_template || ""} onChange={(e) => updateWhatsApp({ grade_template: e.target.value })} /></Field><Field id="whatsapp-revision-template" label="Template revisi"><Textarea id="whatsapp-revision-template" data-testid="whatsapp-revision-template-input" value={w.revision_template || ""} onChange={(e) => updateWhatsApp({ revision_template: e.target.value })} /></Field></div><p className="text-sm text-slate-500" data-testid="whatsapp-template-help">Placeholder: {'{code}'}, {'{minutes}'}, {'{link}'}, {'{name}'}, {'{title}'}, {'{class_name}'}, {'{deadline}'}, {'{grade}'}, {'{predicate}'}, {'{feedback}'}, {'{revision_note}'}</p><Button data-testid="whatsapp-save-button">Simpan Konfigurasi WhatsApp</Button></form>{messages && messages.length > 0 && <Card className="rounded-md shadow-none" data-testid="whatsapp-message-log"><CardHeader><CardTitle data-testid="whatsapp-message-log-title">Log pesan gagal</CardTitle></CardHeader><CardContent className="space-y-2">{messages.filter((m) => ["failed", "pending_config"].includes(m.status)).map((m) => <div key={m.id} className="flex flex-wrap items-center justify-between gap-3 border border-slate-200 p-3" data-testid={`whatsapp-message-row-${m.id}`}><div><p className="font-semibold" data-testid={`whatsapp-message-to-${m.id}`}>{m.to}</p><p className="text-sm text-slate-500" data-testid={`whatsapp-message-error-${m.id}`}>{m.error || m.status}</p></div><Button size="sm" variant="outline" data-testid={`whatsapp-message-retry-${m.id}-button`} onClick={() => retryMessage(m.id)}>Kirim ulang</Button></div>)}</CardContent></Card>}</div>;
}

function EmailPage({ forms, setForms, saveEmail, testEmail }) {
  const e = { ...defaultEmailForm, ...(forms.email || {}) };
  const updateEmail = (changes) => setForms({ ...forms, email: { ...e, ...changes } });
  return <div className="space-y-6" data-testid="email-page"><form onSubmit={saveEmail} className="space-y-4 border bg-white p-5" data-testid="email-settings-form"><div className="flex flex-wrap items-start justify-between gap-4"><div><h2 className="font-display text-2xl font-semibold" data-testid="email-settings-title">Konfigurasi Email (SMTP)</h2><p className="mt-1 text-sm text-slate-500" data-testid="email-settings-subtitle">Kirim OTP reset password dan notifikasi via email.</p></div><Badge className={e.enabled ? "border-emerald-200 bg-emerald-50 text-emerald-700" : "border-slate-200 bg-slate-100 text-slate-600"} data-testid="email-status-badge">{e.enabled ? "Email aktif" : "Email nonaktif"}</Badge></div><div className="grid gap-4 md:grid-cols-2"><div className="flex items-center gap-3"><label className="relative inline-flex cursor-pointer items-center gap-2"><input type="checkbox" className="peer sr-only" data-testid="email-enabled-toggle" checked={e.enabled} onChange={(ev) => updateEmail({ enabled: ev.target.checked })} /><span className="h-6 w-11 rounded-full bg-slate-200 peer-checked:bg-blue-600 after:absolute after:top-[2px] after:left-[2px] after:h-5 after:w-5 after:rounded-full after:bg-white after:transition-all peer-checked:after:translate-x-full" /></label><span className="text-sm font-medium" data-testid="email-enabled-label">{e.enabled ? "Aktif" : "Nonaktif"}</span></div></div><div className="grid gap-4 md:grid-cols-2"><Field id="email-smtp-host" label="SMTP Host"><Input id="email-smtp-host" data-testid="email-smtp-host-input" value={e.smtp_host || ""} onChange={(ev) => updateEmail({ smtp_host: ev.target.value })} placeholder="smtp.gmail.com" /></Field><Field id="email-smtp-port" label="SMTP Port"><Input id="email-smtp-port" type="number" min="1" max="65535" data-testid="email-smtp-port-input" value={e.smtp_port || 587} onChange={(ev) => updateEmail({ smtp_port: Number(ev.target.value) })} /></Field></div><div className="grid gap-4 md:grid-cols-2"><Field id="email-smtp-user" label="SMTP Username"><Input id="email-smtp-user" data-testid="email-smtp-user-input" value={e.smtp_user || ""} onChange={(ev) => updateEmail({ smtp_user: ev.target.value })} placeholder="user@gmail.com" /></Field><Field id="email-smtp-password" label="SMTP Password"><Input id="email-smtp-password" type="password" data-testid="email-smtp-password-input" value={e.smtp_password || ""} onChange={(ev) => updateEmail({ smtp_password: ev.target.value })} placeholder={e.smtp_password_masked || "Password SMTP"} /></Field></div><div className="grid gap-4 md:grid-cols-2"><div className="flex items-center gap-3"><label className="relative inline-flex cursor-pointer items-center gap-2"><input type="checkbox" className="peer sr-only" data-testid="email-tls-toggle" checked={e.smtp_use_tls} onChange={(ev) => updateEmail({ smtp_use_tls: ev.target.checked })} /><span className="h-6 w-11 rounded-full bg-slate-200 peer-checked:bg-blue-600 after:absolute after:top-[2px] after:left-[2px] after:h-5 after:w-5 after:rounded-full after:bg-white after:transition-all peer-checked:after:translate-x-full" /></label><span className="text-sm font-medium" data-testid="email-tls-label">Gunakan TLS</span></div></div><div className="grid gap-4 md:grid-cols-2"><Field id="email-from-name" label="Nama Pengirim"><Input id="email-from-name" data-testid="email-from-name-input" value={e.from_name || ""} onChange={(ev) => updateEmail({ from_name: ev.target.value })} placeholder="E-Learning Dosen" /></Field><Field id="email-from-email" label="Email Pengirim"><Input id="email-from-email" data-testid="email-from-email-input" value={e.from_email || ""} onChange={(ev) => updateEmail({ from_email: ev.target.value })} placeholder="noreply@domain.com" /></Field></div><div className="flex flex-wrap gap-2"><Button data-testid="email-save-button">Simpan Konfigurasi Email</Button><Button type="button" variant="outline" data-testid="email-test-button" onClick={testEmail}>Kirim Email Tes</Button></div></form></div>;
}
...
function AdminApp({ token, user, onLogout, branding, onBrandingUpdate, onUserUpdate }) {
  ...
  async function saveEmail(event) { event.preventDefault(); const operation = progress.begin("Menyimpan konfigurasi email"); try { await axios.put(`${API}/email/settings`, forms.email, auth); await loadAll(); progress.finish(operation, "Konfigurasi email disimpan"); toast.success("Konfigurasi email disimpan"); } catch (error) { const detail = error.response?.data?.detail || "Konfigurasi email gagal"; progress.fail(operation, detail); toast.error(detail); } }
  async function testEmail() { const operation = progress.begin("Mengirim email tes"); try { const { data: result } = await axios.post(`${API}/email/settings/test`, {}, auth); progress.finish(operation, result.message || "Email tes berhasil dikirim"); toast.success(result.message || "Email tes berhasil dikirim"); } catch (error) { const detail = error.response?.data?.detail || "Email tes gagal dikirim"; progress.fail(operation, detail); toast.error(detail); } }
...
const DashboardPage = memo(function DashboardPage({ data, sendReminder, token }) {
...
function DiscussionThread({ material, token }) {
  ...
  function attachmentView(comment) {
    ...
    return <div className="discussion-attachment" data-testid={`discussion-attachment-${comment.id}`}>
      {image && <img src={url} alt={file.file_name || "Lampiran diskusi"} loading="lazy" />}
...
const AssignmentScheduleField = memo(function AssignmentScheduleField({ id, label, value, onChange, description, emptyLabel, required = false, quickActions }) {
    ...
    <div className="admin-material-detail-layout" data-testid="admin-assignment-detail-page">
      ...
      <div className="space-y-4" data-testid="assignment-list">
        {selectedGroup.assignments.length === 0 ? <EmptyState title="Belum ada tugas" description="Tambahkan tugas pertama untuk mata kuliah ini." /> : selectedGroup.assignments.map((item) => <Card key={item.id} className="overflow-hidden rounded-md shadow-none" data-testid={`assignment-card-${item.id}`}>
          {item.is_practicum && <img src={practicumCover} alt="Mode praktikum" className="h-36 w-full object-cover" loading="lazy" data-testid={`assignment-practicum-image-${item.id}`} />}
...
const FilePreviewPanel = memo(function FilePreviewPanel({ previewTarget, previewDoc, previewError, previewBusy, token, onClose }) {
...
function GradingPage({ data, forms, setForms, gradeSubmission, bulkGradeSubmissions, markReviewed, requestRevision, token }) {
  ...
  const classGroups = useMemo(() => {
    ...
    (data.assignments || []).forEach((a) => {
      ...
      if (!map.has(cid)) map.set(cid, { classId: cid, courseName: a.course_name || "Mapel", className: a.class_name || "Kelas", assignments: 0, total: 0, ungraded: 0, graded: 0 });
      const g = map.get(cid);
      g.assignments += 1;
    });
    (data.submissions || []).forEach((s) => {
      ...
      if (!map.has(cid)) map.set(cid, { classId: cid, courseName: assignment.course_name || "Mapel", className: assignment.class_name || "Kelas", assignments: 0, total: 0, ungraded: 0, graded: 0 });
      const g = map.get(cid);
      if (["Sudah Submit", "Terlambat", "Direvisi", "Dinilai"].includes(s.status)) {
        g.total += 1;
        if (s.grade != null) g.graded += 1; else g.ungraded += 1;
      }
    });
    classOptions.forEach((c) => {
      if (!map.has(c.id)) map.set(c.id, { classId: c.id, courseName: c.course_name || "Mapel", className: c.name || "Kelas", assignments: 0, total: 0, ungraded: 0, graded: 0 });
    });
    return Array.from(map.values()).sort((a, b) => a.courseName.localeCompare(b.courseName, "id"));
  }, [data.assignments, data.submissions, classOptions]);

  const selectedAssignments = useMemo(() => (data.assignments || []).filter((a) => a.class_id === selectedClassId), [data.assignments, selectedClassId]);
  const visibleAssignments = selectedAssignments.filter((a) => !filter.assignment_id || a.id === filter.assignment_id);
  const ready = useMemo(() => {
  ...
  }, [data.submissions, data.assignments, selectedClassId, filter.assignment_id]);
  ...
  useEffect(() => {
    if (selectedClassId && !classOptions.some((c) => c.id === selectedClassId)) setSelectedClassId("");
  ...
  if (!selectedClass) {
    return <div className="space-y-5" data-testid="grading-page">
      <section className="meeting-hero" data-testid="grading-hero">
        <div>
          <p className="meeting-overline">Penilaian</p>
          <h2 className="font-display text-2xl font-semibold" data-testid="grading-overview-title">Penilaian per kelas</h2>
          <p className="meeting-description">Pilih kelas untuk melihat submission mahasiswa dan mengisi nilai.</p>
        </div>
        <div className="meeting-summary" data-testid="grading-overview-summary">
          <div><strong>{classGroups.length}</strong><span>Kelas</span></div>
          <div><strong>{(data.assignments || []).length}</strong><span>Tugas</span></div>
          <div><strong>{(data.submissions || []).filter((s) => s.grade != null).length}</strong><span>Dinilai</span></div>
        </div>
      </section>
      {classGroups.length === 0 ? <EmptyState title="Belum ada kelas" description="Buat kelas dan tugas terlebih dahulu." /> : <div className="course-card-grid" data-testid="grading-class-grid">
        {classGroups.map((g) => <Card key={g.classId} className="course-material-card rounded-md shadow-none cursor-pointer hover:shadow-md transition-shadow" data-testid={`grading-class-card-${g.classId}`} onClick={() => openClass(g.classId)}>
          <CardContent className="course-material-card-content">
            <div className="course-material-card-main">
              <span className="course-material-card-icon"><CheckCircle2 /></span>
              <div className="course-material-card-header">
                <div>
                  <h3 data-testid={`grading-class-course-${g.classId}`}>{g.courseName}</h3>
                  <p data-testid={`grading-class-name-${g.classId}`}>{g.className}</p>
                </div>
              </div>
            </div>
            <div className="course-material-card-meta" data-testid={`grading-class-meta-${g.classId}`}>
              <span>{g.assignments} tugas</span>
              <span>{g.total} submission</span>
              {g.ungraded > 0 && <span className="text-amber-600">{g.ungraded} belum dinilai</span>}
              {g.ungraded === 0 && g.total > 0 && <span className="text-emerald-600">Semua dinilai</span>}
            </div>
          </CardContent>
        </Card>)}
      </div>}
  ...
  return <div className="space-y-6" data-testid="grading-detail-page">
    <div className="flex flex-wrap items-center gap-3" data-testid="grading-detail-header">
      <Button variant="outline" size="sm" data-testid="grading-back-button" onClick={backToOverview}><ArrowLeft /> Kembali</Button>
      <div>
        <p className="text-xs font-semibold uppercase tracking-[0.2em] text-slate-500" data-testid="grading-detail-overline">Penilaian</p>
        <h2 className="font-display text-2xl font-bold text-slate-950" data-testid="grading-detail-title">{selectedClass.courseName} <span className="text-slate-400">·</span> {selectedClass.className}</h2>
      </div>
    </div>
    <Card className="rounded-md shadow-none" data-testid="grading-filter-card"><CardContent className="grid gap-3 p-4 md:grid-cols-2">
      <Field id="grading-filter-assignment" label="Filter tugas"><select id="grading-filter-assignment" className="form-select" data-testid="grading-filter-assignment-select" value={filter.assignment_id} onChange={(e) => setFilter({ ...filter, assignment_id: e.target.value })}><option value="">Semua tugas</option>{selectedAssignments.map((a) => <option key={a.id} value={a.id}>{a.title}</option>)}</select></Field>
      <div className="flex items-end gap-3 text-sm text-slate-500" data-testid="grading-detail-stats">
        <span>{ready.length} submission</span>
        <span>{ready.filter((s) => s.grade != null).length} dinilai</span>
        <span>{ready.filter((s) => s.grade == null).length} belum dinilai</span>
      </div>
    </CardContent></Card>
...
const GRADE_CHART_COLORS = { A: "#10b981", B: "#3b82f6", C: "#f59e0b", D: "#f97316", E: "#ef4444" };
...
const GradeRecapPage = memo(function GradeRecapPage({ data }) {
  ...
  if (!selectedClass) {
    return <div className="space-y-5" data-testid="grade-recap-page">
      <section className="meeting-hero" data-testid="grade-recap-hero">
        <div>
          <p className="meeting-overline">Rekap Nilai</p>
          <h2 className="font-display text-2xl font-semibold" data-testid="grade-recap-title">Rekap nilai per mata kuliah</h2>
          <p className="meeting-description">Lihat rata-rata nilai, distribusi grade, dan grafik nilai mahasiswa per kelas.</p>
        </div>
        <div className="meeting-summary" data-testid="grade-recap-summary">
          <div><strong>{recapData.length}</strong><span>Kelas</span></div>
          <div><strong>{recapData.reduce((sum, c) => sum + c.student_count, 0)}</strong><span>Mahasiswa</span></div>
        </div>
      </section>
      {recapData.length === 0
        ? <EmptyState title="Belum ada rekap nilai" description="Nilai dari tugas yang sudah dinilai akan muncul di sini." />
        : <div className="course-card-grid" data-testid="grade-recap-class-grid">
            {recapData.map((g) =>
              <Card key={g.class_id} className="course-material-card rounded-md shadow-none cursor-pointer hover:shadow-md transition-shadow" data-testid={`grade-recap-card-${g.class_id}`} onClick={() => setSelectedClass(g)}>
                <CardContent className="course-material-card-content">
                  <div className="course-material-card-main">
                    <span className="course-material-card-icon"><BarChart3 /></span>
                    <div className="course-material-card-header">
                      <div>
                        <h3 data-testid={`grade-recap-course-${g.class_id}`}>{g.course_name}</h3>
                        <p data-testid={`grade-recap-class-${g.class_id}`}>{g.class_name}</p>
                      </div>
                    </div>
                  </div>
                  <div className="course-material-card-meta" data-testid={`grade-recap-meta-${g.class_id}`}>
                    <span>{g.student_count} mahasiswa</span>
                    <span>{g.total_assignments} tugas</span>
                    <span>Rata-rata: <strong className="text-emerald-600">{g.class_average}</strong></span>
                  </div>
                </CardContent>
              </Card>
            )}
  ...
  const { students, grade_distribution: dist, class_average: classAvg } = selectedClass;
  const allAssignmentIds = [...new Set(students.flatMap((s) => s.scores.map((sc) => sc.assignment_id)))];
  const assignmentTitles = {};
  students.forEach((s) => s.scores.forEach((sc) => { assignmentTitles[sc.assignment_id] = sc.assignment_title; }));
  const allAssignments = allAssignmentIds.map((id) => ({ id, title: assignmentTitles[id] || "Tugas" }));

  const chartData = [...students]
    .sort((a, b) => b.average - a.average)
    .map((s) => ({ name: s.student_name.split(" ").slice(0, 2).join(" "), average: s.average, nim: s.student_nim }));

  const distData = Object.entries(dist).map(([name, value]) => ({ name, value, fill: GRADE_CHART_COLORS[name] || "#94a3b8" }));

  return <div className="space-y-6" data-testid="grade-recap-detail">
    <div className="flex flex-wrap items-center gap-3" data-testid="grade-recap-detail-header">
      <Button variant="outline" size="sm" data-testid="grade-recap-back-button" onClick={() => setSelectedClass(null)}><ArrowLeft /> Kembali</Button>
      <div>
        <p className="text-xs font-semibold uppercase tracking-[0.2em] text-slate-500">Rekap Nilai</p>
        <h2 className="font-display text-2xl font-bold text-slate-950" data-testid="grade-recap-detail-title">{selectedClass.course_name} <span className="text-slate-400">·</span> {selectedClass.class_name}</h2>
      </div>
    </div>

    <div className="grid gap-4 md:grid-cols-4" data-testid="grade-recap-stats">
      <StatCard icon={Users} label="Mahasiswa" value={selectedClass.student_count} hint="Terdaftar di kelas ini" testid="recap-stat-students" />
      <StatCard icon={ClipboardList} label="Tugas dinilai" value={selectedClass.total_assignments} hint="Total tugas yang sudah dinilai" testid="recap-stat-assignments" />
      <StatCard icon={BarChart3} label="Rata-rata kelas" value={classAvg} hint="Nilai rata-rata seluruh mahasiswa" testid="recap-stat-average" />
      <StatCard icon={GraduationCap} label="Nilai tertinggi" value={Math.max(...students.map((s) => s.average), 0)} hint="Rata-rata tertinggi mahasiswa" testid="recap-stat-max" />
    </div>

    <div className="grid gap-6 xl:grid-cols-2" data-testid="grade-recap-charts">
      <Card className="rounded-md shadow-none" data-testid="recap-chart-student-card">
        <CardHeader><CardTitle data-testid="recap-chart-student-title">Rata-rata nilai per mahasiswa</CardTitle></CardHeader>
        <CardContent>
          {chartData.length === 0
            ? <p className="text-sm text-slate-500">Belum ada data nilai.</p>
            : <ResponsiveContainer width="100%" height={Math.max(200, Math.min(500, students.length * 36))}>
                <BarChart data={chartData} layout="vertical" margin={{ top: 5, right: 20, left: 100, bottom: 5 }}>
                  <CartesianGrid strokeDasharray="3 3" />
                  <XAxis type="number" domain={[0, 100]} tick={{ fontSize: 12 }} />
                  <YAxis type="category" dataKey="name" width={90} tick={{ fontSize: 12 }} />
                  <Tooltip formatter={(value, name, props) => [`${value} (${props.payload.nim})`, "Rata-rata"]} />
      ...
      <Card className="rounded-md shadow-none" data-testid="recap-chart-dist-card">
        <CardHeader><CardTitle data-testid="recap-chart-dist-title">Distribusi grade</CardTitle></CardHeader>
        <CardContent>
          <ResponsiveContainer width="100%" height={300}>
            <BarChart data={distData}>
              ...
              <Bar dataKey="value" name="Jumlah mahasiswa" radius={[4, 4, 0, 0]}>
                {distData.map((entry, index) => <Cell key={`cell-${index}`} fill={entry.fill || DIST_COLORS[index % DIST_COLORS.length]} />)}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
        </CardContent>
    ...
    <Card className="rounded-md shadow-none" data-testid="recap-table-card">
      <CardHeader><CardTitle data-testid="recap-table-title">Detail nilai mahasiswa</CardTitle></CardHeader>
      <CardContent>
          ...
          <Table data-testid="recap-students-table">
            <TableHeader>
              <TableRow>
                ...
                {allAssignments.map((a) => <TableHead key={a.id} className="whitespace-nowrap text-center">{a.title}</TableHead>)}
                <TableHead className="whitespace-nowrap text-center">Rata-rata</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {students.map((s) => {
                const scoreMap = {};
                s.scores.forEach((sc) => { scoreMap[sc.assignment_id] = sc; });
                return <TableRow key={s.student_id} data-testid={`recap-student-row-${s.student_id}`}>
                  <TableCell className="font-mono text-sm whitespace-nowrap">{s.student_nim}</TableCell>
                  <TableCell className="font-medium whitespace-nowrap">{s.student_name}</TableCell>
                  {allAssignments.map((a) => {
                    const sc = scoreMap[a.id];
                    return <TableCell key={a.id} className="text-center whitespace-nowrap">
                      {sc
                        ? <Badge className={sc.grade >= 70 ? "border-emerald-200 bg-emerald-50 text-emerald-700" : "border-amber-200 bg-amber-50 text-amber-700"}>{sc.grade}</Badge>
                        : <span className="text-slate-300">-</span>
                      }
                    </TableCell>;
                  })}
                  <TableCell className="text-center font-bold whitespace-nowrap">{s.average}</TableCell>
                </TableRow>;
              })}
            </TableBody>
          </Table>
...
function StudentApp({ token, user, onLogout, branding, onUserUpdate }) {
  ...
  return <div className="min-h-screen bg-slate-50 text-slate-900" data-testid="student-app-shell">
    <aside className="student-sidebar fixed inset-y-0 left-0 z-30 hidden w-72 bg-slate-950 p-5 text-white lg:block" data-testid="student-sidebar">
      ...
      <div className="mt-6 border-t border-white/10 pt-4" data-testid="student-sidebar-join-class">
        <p className="mb-2 text-xs font-semibold uppercase tracking-widest text-slate-400">Gabung Kelas</p>
        <form onSubmit={requestJoinClass} className="space-y-2" data-testid="student-sidebar-join-form">
          <Input className="w-full bg-white/10 text-white placeholder:text-slate-400" data-testid="student-sidebar-class-code-input" value={classCode} onChange={(e) => setClassCode(e.target.value)} placeholder="Masukkan kode kelas" />
          <Button type="submit" size="sm" className="w-full" data-testid="student-sidebar-join-submit-button"><Send /> Gabung</Button>
        </form>
        {data.enrollments.length > 0 && <div className="mt-3 space-y-1" data-testid="student-sidebar-enrollment-list">{data.enrollments.map((r) => <div key={r.id} className="flex items-center gap-2 text-xs text-slate-300" data-testid={`student-sidebar-enrollment-${r.id}`}><span className={`inline-block h-2 w-2 rounded-full ${r.status === "approved" ? "bg-emerald-400" : "bg-amber-400"}`} /><span className="truncate">{r.class_name}</span><span className="ml-auto shrink-0 text-slate-500">{r.status === "approved" ? "Aktif" : "Proses"}</span></div>)}</div>}</div>
    </aside>
    <main className="student-main-content lg:pl-72">
      ...
      <section className="student-page-content space-y-6 p-5 md:p-8" data-testid="student-dashboard-section">
        {studentPage === "home" && <>
          <Card className="rounded-md shadow-none lg:hidden" data-testid="student-mobile-join-card"><CardContent className="p-5"><form onSubmit={requestJoinClass} className="flex flex-wrap gap-3" data-testid="student-mobile-join-form"><Input className="max-w-xs flex-1" data-testid="student-mobile-class-code-input" value={classCode} onChange={(e) => setClassCode(e.target.value)} placeholder="Kode kelas" /><Button data-testid="student-mobile-join-submit-button"><Send /> Gabung</Button></form><div className="mt-3 flex flex-wrap gap-2" data-testid="student-mobile-enrollment-list">{data.enrollments.map((r) => <Badge key={r.id} className={statusClass(r.status === "approved" ? "Aman" : "Risiko Rendah")} data-testid={`student-mobile-enrollment-${r.id}`}>{r.class_name}: {r.status}</Badge>)}</div></CardContent></Card>
          {(() => {
            const approvedEnrollments = data.enrollments.filter((r) => r.status === "approved");
            const classMap = new Map();
            data.assignments.forEach((a) => {
              const key = a.class_id || a.class_name;
              if (!key) return;
              if (!classMap.has(key)) classMap.set(key, { class_name: a.class_name || "Kelas", course_name: a.course_name || "Mata kuliah", assignmentCount: 0, submitted: 0 });
              const entry = classMap.get(key);
              entry.assignmentCount += 1;
              if (a.my_submission) entry.submitted += 1;
            });
            approvedEnrollments.forEach((r) => {
              if (!classMap.has(r.class_id)) classMap.set(r.class_id, { class_name: r.class_name || "Kelas", course_name: "", assignmentCount: 0, submitted: 0 });
            });
            const classes = Array.from(classMap.values());
            return <Card className="rounded-md shadow-none" data-testid="student-my-classes-card"><CardHeader><CardTitle data-testid="student-my-classes-title">Kelas & Mata Kuliah Saya</CardTitle></CardHeader><CardContent>{classes.length === 0 ?                 <p className="text-sm text-slate-500" data-testid="student-my-classes-empty">Belum ada kelas aktif. Masukkan kode kelas di sidebar untuk bergabung.</p> : <div className="grid gap-3 md:grid-cols-2" data-testid="student-my-classes-grid">{classes.map((cls, idx) => <div key={idx} className="flex items-start gap-3 rounded-lg border border-slate-200 p-4" data-testid={`student-class-card-${idx}`}><div className="rounded-lg bg-blue-50 p-2 text-blue-700"><BookOpen className="h-5 w-5" /></div><div className="flex-1 min-w-0"><p className="font-semibold truncate" data-testid={`student-class-course-${idx}`}>{cls.course_name}</p><p className="text-sm text-slate-500 truncate" data-testid={`student-class-name-${idx}`}>{cls.class_name}</p></div>{cls.assignmentCount > 0 && <Badge className="shrink-0 border-blue-200 bg-blue-50 text-blue-700" data-testid={`student-class-assignment-count-${idx}`}>{cls.submitted}/{cls.assignmentCount} tugas</Badge>}</div>)}</div>}</CardContent></Card>;
          })()}
    ...
    {selected && <div className="chat-conversation">
      ...
      <div className="chat-messages" data-testid="chat-message-list">
        ...
        {messages.map((message) => <article key={message.id} className={`chat-bubble ${message.sender_id === user.id ? "mine" : ""}`} data-testid={`chat-message-${message.id}`}>
          {message.attachment && <img src={`${BACKEND_URL}${message.attachment.inline_url}?token=${encodeURIComponent(token)}`} alt="Lampiran chat" loading="lazy" />}
...
function App() {
  ...
  const handleAuth = useCallback((payload) => { localStorage.setItem("elearn_token", payload.token); localStorage.setItem("elearn_user", JSON.stringify(payload.user)); setToken(payload.token); setUser(payload.user); }, []);
  const handleUserUpdate = useCallback((updated) => { localStorage.setItem("elearn_user", JSON.stringify(updated)); setUser(updated); }, []);
  const handleBrandingUpdate = useCallback((updated) => { setBranding((current) => ({ ...current, ...updated })); }, []);
  const logout = useCallback(() => { localStorage.removeItem("elearn_token"); localStorage.removeItem("elearn_user"); setToken(""); setUser(null); }, []);
  return (
